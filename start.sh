...
#
# ES 는 평문 HTTP 로 운영한다 (docker-compose-dep.yml 의 xpack.security.http.ssl.enabled=false).
# 9200 은 보안그룹에서 닫혀 있고 애플리케이션은 도커 네트워크 안에서만 붙으므로 TLS 를 걸지 않는다.
# 여기서 https 로 물으면 매번 실패해서 5 분을 헛되이 대기한다.
log "Waiting for Elasticsearch..."
ELASTIC_PW=$(grep '^ELASTIC_PASSWORD=' "$COMPOSE_DIR/.env" | cut -d= -f2)
ES_READY=false
for i in $(seq 1 60); do
  if docker exec omisys-es01-1 curl -sf http://localhost:9200 \
...
# 3. Kafka 헬스체크 대기 (최대 3분)
#
# apache/kafka 이미지는 CLI 를 PATH 에 넣지 않는다. 전체 경로로 불러야 한다.
log "Waiting for Kafka..."
KAFKA_READY=false
for i in $(seq 1 36); do
  if docker exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server kafka:9092 --list > /dev/null 2>&1; then
